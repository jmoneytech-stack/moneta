...
Phase 2 is complete.
Phase 3's D3-1 liability-sign and D3-2 nullable-money foundation is complete, PR3 adds compute-on-read `networth --history`, and PR4-PR7 add the `mom`, `merchants`, `utilization`, and `savings` trend metrics; all reads have authenticated REST mirrors.
Do not begin PR8 fixed-variable or any later Phase 3 feature until explicitly requested by the maintainer; PR8's R3 definition remains unresolved.
