...
`moneta status`, `moneta accounts`, `moneta tx`, `moneta spend`, `moneta cashflow`, and `moneta networth` ship on the shared TOON/JSON output path (`internal/toon`, `internal/cli`).
